    ...
    try {
      ...
      if (heartbeatResult.success) {
        ...
        if (this.heartbeatAttempts < 3) {
          const delay = this.heartbeatAttempts * 3000; // 递增延迟：3s, 6s, 9s
          return {
            success: false,
            nextState: DeviceState.HEARTBEAT,
            reason: `Heartbeat failed (attempt ${this.heartbeatAttempts}), retrying`,
            retryDelay: delay
          };
        } else {
          return {
            ...
            reason: 'Heartbeat failed after maximum retries'
          };
        }
      }
