...
export class EmployeeMonitorApp extends EventEmitter {
   ...
   * 处理系统唤醒事件
   *
   * 方案 C 重构：使用 reconnect() 而非 connect()
   * reconnect() 会重置计数器并销毁旧连接，确保系统唤醒后有全新的重连机会
   */
  private async handleSystemResume(event: {
    ...
    logger.info('[APP] 🌅 Handling system resume', {
      ...
      suspendDurationMs: event.suspendDuration
    });

    // 等待 3 秒让网络稳定（从 2 秒增加到 3 秒，提高网络就绪成功率）
    logger.info('[APP] Waiting 3s for network to stabilize...');
    ...
    if (wsService) {
      ...
      if (!isConnected) {
        ...
        try {
          // 方案 C：使用 reconnect() 而非 connect()
          // reconnect() 会重置计数器并完全销毁旧连接
          await wsService.reconnect();
          logger.info('[APP] ✅ WebSocket reconnected successfully after system resume');
        } catch (error) {
          logger.error('[APP] ❌ Failed to reconnect WebSocket after resume:', error);
          // Socket.IO 内置重连机制会继续尝试（reconnectionAttempts: Infinity）
          logger.info('[APP] Socket.IO built-in reconnection will continue attempting...');
        }
      } else {
        logger.info('[APP] ✅ WebSocket already connected after resume');
      }
    }
