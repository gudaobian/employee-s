...
export class ActivityCollectorService extends BaseService {
  ...
  // CRITICAL FIX: 保存绑定的监听器引用，便于清理
  private boundConfigUpdateHandler?: (updatedConfig: any) => Promise<void>;
  ...
  async stop(): Promise<void> {
    ...
    try {
      ...
      // CRITICAL FIX: 清理 URL 采集服务
      if (this.urlCollectorService) {
        await this.urlCollectorService.destroy();
      }
  ...
  private setupConfigChangeListener(): void {
    // CRITICAL FIX: 使用绑定的引用，便于后续清理
    // 避免匿名函数导致的监听器泄漏
    this.boundConfigUpdateHandler = async (updatedConfig: any) => {
    ...
    // 监听配置服务的变更事件
    this.configService.on?.('config-updated', this.boundConfigUpdateHandler);
  }

  /**
   * CRITICAL FIX: 清理配置监听器
   */
  private removeConfigChangeListener(): void {
    if (this.boundConfigUpdateHandler && (this.configService as any).off) {
      (this.configService as any).off('config-updated', this.boundConfigUpdateHandler);
      this.boundConfigUpdateHandler = undefined;
      logger.info('[ACTIVITY_COLLECTOR] Config change listener removed');
    }
  }
