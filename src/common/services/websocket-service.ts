...
export class WebSocketService extends EventEmitter implements IWebSocketService {
  ...
  // NOTE: 移除了 maxReconnectAttempts 和 reconnectDelay
  // 方案 C 重构：传输层重连完全由 Socket.IO 内置机制处理（reconnectionAttempts: Infinity）
  // 应用层重连（系统唤醒、配置变更）由 reconnect() 方法处理
  ...
  private stats: ConnectionStats = {
    ...
    messagesReceived: 0,
    reconnectAttempts: 0,
    maxReconnectAttempts: Infinity  // Socket.IO 内置无限重试
  };
  ...
  private setupEventListeners(): void {
    ...
    this.socket.on('disconnect', (reason: string) => {
      ...
      // 方案 C 重构：不再调用自定义 scheduleReconnect()
      // Socket.IO 内置重连机制会自动处理传输层重连（reconnection: true, reconnectionAttempts: Infinity）
      // 应用层重连（系统唤醒、配置变更）由外部调用 reconnect() 方法处理
      if (this.shouldReconnect && reason !== 'io client disconnect') {
        console.log('[WEBSOCKET] Socket.IO will handle reconnection automatically');
      }
    });
  ...
  // NOTE: scheduleReconnect() 方法已移除
  // 方案 C 重构：传输层重连由 Socket.IO 内置机制处理
  // 应用层重连由 reconnect() 方法处理
  ...
  /**
   * 手动触发重连（用于系统唤醒、配置变更等应用层场景）
   *
   * 方案 C 重构：此方法是应用层主动重连的唯一入口
   * - 重置重连计数器，给予全新的重连机会
   * - 完全销毁旧连接，避免残留监听器
   * - 创建全新的 Socket.IO 连接
   */
  async reconnect(): Promise<void> {
    console.log('[WEBSOCKET] 🔄 Manual reconnection triggered (application layer)');

    // 重置计数器，给予全新的重连机会
    this.reconnectAttempts = 0;
    this.stats.reconnectAttempts = 0;
    console.log('[WEBSOCKET] Reconnect attempts counter reset to 0');
    ...
    // 完全销毁旧连接
    if (this.socket) {
      console.log('[WEBSOCKET] Destroying old socket connection...');

      // 暂时禁用旧 socket 的自动重连，避免与新连接冲突
      ...
      console.log('[WEBSOCKET] Old socket destroyed');
    }
