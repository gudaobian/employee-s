...
import { TeachingModeService, TeachingModeConfig } from '../../teaching-mode-service';
...
export class DataCollectStateHandler extends BaseStateHandler {
  ...
  private isTeachingMode = false;

  // 教学模式下的配置 (低干扰)
  private teachingModeConfig = {
    ...
    screenshotPriority: 'low' as const,
   ...
   * 初始化教学模式检测服务
   */
  private initTeachingModeService(): void {
    try {
      this.teachingModeService = new TeachingModeService(this.platformAdapter);

      // 监听模式变更事件
      this.teachingModeService.on('mode-changed', (event) => {
        this.handleTeachingModeChange(event);
      });

      // 启动检测
      this.teachingModeService.start();

      logger.info('[DATA_COLLECT] 🎓 Teaching mode service initialized');
    } catch (error) {
      logger.warn('[DATA_COLLECT] Failed to initialize teaching mode service:', error);
      // 不阻止其他功能
    }
  }
   ...
   * 处理教学模式变更
   */
  private handleTeachingModeChange(event: {
    oldMode: string;
    newMode: string;
    detectedApp: string | null;
    config: any;
  }): void {
    const wasTeachingMode = this.isTeachingMode;
    this.isTeachingMode = event.newMode === 'teaching';

    logger.info(`[DATA_COLLECT] 🔄 Teaching mode changed: ${event.oldMode} → ${event.newMode}`, {
    ...
    if (this.isTeachingMode) {
      logger.info('[DATA_COLLECT] 🎓 Entering teaching mode - applying low-interference settings', {
        screenshotInterval: `${this.teachingModeConfig.screenshotInterval / 60000}分钟`,
        screenshotQuality: `${this.teachingModeConfig.screenshotQuality}%`,
        screenshotResolution: `${this.teachingModeConfig.screenshotMaxWidth}x${this.teachingModeConfig.screenshotMaxHeight}`,
        processScanInterval: `${this.teachingModeConfig.processScanInterval / 60000}分钟`,
        gcEnabled: 'false (only on idle)'
      });
    } else {
      logger.info('[DATA_COLLECT] 📊 Exiting teaching mode - restoring normal settings');
    }

    // 注意: 定时器间隔的调整在下次触发时自动生效
    // 不需要重启定时器，因为 performScreenshotCollection 会根据 isTeachingMode 选择配置
  }
  ...
  private async performScreenshotCollection(): Promise<void> {
    try {
      ...
      logger.info(`[DATA_COLLECT] 截图采集启用状态: ${isScreenshotEnabled}, 教学模式: ${this.isTeachingMode}`);
      ...
      // 🎓 教学模式智能配置选择
      // 教学模式下使用低干扰配置：更低质量、更小分辨率、更大随机延迟、低优先级
      let screenshotConfig: any;

      if (this.isTeachingMode) {
        // 教学模式 - 低干扰配置
        screenshotConfig = {
          enabled: true,
          quality: this.teachingModeConfig.screenshotQuality,           // 5%
          format: 'jpg',
          maxWidth: this.teachingModeConfig.screenshotMaxWidth,         // 960
          maxHeight: this.teachingModeConfig.screenshotMaxHeight,       // 540
          randomDelay: this.teachingModeConfig.screenshotRandomDelay,   // 10秒
          priority: this.teachingModeConfig.screenshotPriority          // 'low'
        };

        logger.info('[DATA_COLLECT] 🎓 使用教学模式截图配置', {
          quality: `${screenshotConfig.quality}%`,
          resolution: `${screenshotConfig.maxWidth}x${screenshotConfig.maxHeight}`,
          randomDelay: `${screenshotConfig.randomDelay}ms`,
          priority: screenshotConfig.priority
        });
      } else {
        // 正常模式 - 标准配置
        screenshotConfig = {
        ...
        logger.info('[DATA_COLLECT] 📊 使用正常模式截图配置');
      }
  ...
  public cleanup(): void {
    try {
      ...
      // Stop teaching mode service
      if (this.teachingModeService) {
        this.teachingModeService.stop();
        this.teachingModeService.removeAllListeners();
        this.teachingModeService = undefined;
        logger.info('[DATA_COLLECT] ✅ Teaching mode service stopped');
      }
