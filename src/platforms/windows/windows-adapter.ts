...
import WindowsNativeEventAdapter from './native-event-adapter';
...
`.trim();
  ...
  async takeScreenshot(options: ScreenshotOptions = {}): Promise<ScreenshotResult> {
    ...
    try {
      ...
      // 教学模式支持: 随机延迟和低优先级
      const randomDelayMax = (options as any).randomDelay || 0;
      const isLowPriority = (options as any).priority === 'low';

      // 随机延迟 (分散截图时机，减少与屏幕共享冲突)
      if (randomDelayMax > 0) {
        const delay = Math.floor(Math.random() * randomDelayMax);
        logger.debug(`[WINDOWS] Screenshot random delay: ${delay}ms (max: ${randomDelayMax}ms)`);
        await new Promise(r => setTimeout(r, delay));
      }

      if (isLowPriority) {
        logger.info('[WINDOWS] 📸 Taking screenshot with LOW priority (teaching mode)');
        // Windows 下通过 SetPriorityClass 降低进程优先级的效果有限
        // 但随机延迟已经能有效分散负载
      }

      logger.info(`[WINDOWS] 开始截图... (质量: ${quality}, 格式: ${format}, 优先级: ${isLowPriority ? 'low' : 'normal'})`);
      ...
      try {
        ...
        const metadataInstance = sharp(imgBuffer);
        const metadata = await metadataInstance.metadata();
        metadataInstance.destroy(); // 释放 libvips 资源
