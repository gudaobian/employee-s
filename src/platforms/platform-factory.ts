...
export type PlatformType = 'darwin' | 'win32' | 'unknown';
...
export class PlatformFactory {
  ...
  async createPlatformAdapter(): Promise<IPlatformAdapter> {
    ...
    switch (platformInfo.type) {
      case 'darwin': {
        const { default: MacOSAdapter } = await import('./macos/macos-adapter');
        this.currentAdapter = new MacOSAdapter();
        break;
      }
  ...
  isPlatformSupported(platform?: PlatformType): boolean {
    ...
    return ['darwin', 'win32'].includes(targetPlatform);
  }
