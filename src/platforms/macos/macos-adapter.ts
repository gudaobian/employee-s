...
export class MacOSAdapter extends PlatformAdapterBase {
  ...
  async takeScreenshot(options: any = {}): Promise<any> {
    ...
    try {
      ...
      // 教学模式支持: 随机延迟和低优先级
      const randomDelayMax = options.randomDelay || 0;
      const isLowPriority = options.priority === 'low';

      // 随机延迟 (分散截图时机，减少与屏幕共享冲突)
      if (randomDelayMax > 0) {
        const delay = Math.floor(Math.random() * randomDelayMax);
        logger.debug(`[DARWIN] Screenshot random delay: ${delay}ms (max: ${randomDelayMax}ms)`);
      ...
      // -x: 禁用截图声音
      // -t png: 指定输出格式为 PNG
      // nice -n 19: 最低优先级，让出 CPU 时间给其他进程 (教学模式)
      const nicePrefix = isLowPriority ? 'nice -n 19 ' : '';
      let command = `${nicePrefix}screencapture -x -t png "${tempPngPath}"`;
      if (options.displayId !== undefined) {
        command += ` -D ${options.displayId}`;
      }

      if (isLowPriority) {
        logger.info('[DARWIN] 📸 Taking screenshot with LOW priority (teaching mode)');
      }
